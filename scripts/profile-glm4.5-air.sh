...
# Set default values for num_reqs and max_tokens (in units of 1024 tokens)
num_reqs="500"
max_tokens="32" # 32k tokens
batch_sizes="1 2 4"
...
do
    for method in deepseek_mtp ngram none
    do
        ...
        if [ "$method" = "none" ]; then
            ...
            spec_tokens_list="3"
        elif [ "$method" = "deepseek_mtp" ]; then
            spec_tokens_list="3"
        fi

        # set num_reqs based on dataset size
        if [ "$dataset" = "aime" ]; then
            num_reqs="90"
        elif [ "$dataset" = "gpqa_main" ]; then
            num_reqs="448"
        fi
