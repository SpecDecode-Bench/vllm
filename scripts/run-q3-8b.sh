#!/bin/bash
# Profile Qwen/Qwen3-8B (thinking enabled) with all speculative decoding methods,
# then generate speedup plots.
# Usage: bash run-q3-8b.sh
...
model="Qwen/Qwen3-8B"
num_reqs=500
max_tokens=32  # 32k tokens; thinking mode produces long outputs
...
for dataset in instructcoder cnndailymail sharegpt gsm8k; do
    for method in none ngram eagle3; do
...
python vis_speedup.py --results_dir "$output_dir" --model_key qwen3-8b
