...
# VLLM_PATH="/data/lily/draft-sd-vllm/vllm/benchmarks"
# # VLLM_PATH="/data/lily/sd-benchmark-paper/vllm-benchmark/benchmarks/"
# sys.path.append(VLLM_PATH)
SEED=42

from vllm.benchmarks.datasets_bench import (AIMODataset,
                            ShareGPTDataset,
                            SonnetDataset,
                            MTBenchDataset,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Draft model name or path (if different from the base model).",
    )
    parser.add_argument(
        "--max_model_len",
        type=int,
        default=8192,
        help="Align draft-target maximum model context length (in tokens).",
    )
